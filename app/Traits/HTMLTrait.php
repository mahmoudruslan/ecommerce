...
trait HTMLTrait {
    ...
    public function vgetModal($route, $row)
    {
        return '<div class="modal fade" id="DeleteModal'. $row->id .'" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">'.__("Are you sure you want to delete the item?").'</div>
                    <form action="'. route($route, [$row->product, $row]).'" method="post">
                    '.method_field("DELETE").'
                    <input type="hidden" name="_token" value="'.csrf_token().'" />
                    <div class="modal-footer">
                        <button class="btn btn-secondary" type="button" data-dismiss="modal">' .__("Cancel").'</button>
    ...
    public function vgetEditLink($route, $row)
    {
        return '<div role="group" aria-label="Basic example" class="btn-group" style="width: 150px"> <a type="button" href=" ' . route($route, [$row->product, $row]) . '" class="rbtn btn btn-primary btn-sm"><i class="fas fa-fw fa-edit"></i></a>';
    }
    ...
    public function vgetShowLink($route, $row)
    {
        return '<a type="button" href=" ' . route($route, [$row->product, $row]) . '" class=" btn btn-warning btn-sm"><i class="fas fa-fw fa-eye"></i></a>';
    }
    ...
    public function vgetDeleteLink($route, $row)
    {
        $btn = ' <a type="button" href="javascript:void(0)" data-toggle="modal" data-target="#DeleteModal'. $row->id.'" class="lbtn btn btn-danger btn-sm"><i class="fas fa-fw fa-trash"></i></a></div>';
        $btn = $btn. $this->vgetModal($route, $row);
        return $btn;
    }
